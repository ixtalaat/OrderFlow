...
public sealed class CompleteOrderCommandHandler(IOrderRepository orders, IInventoryRepository inventories, IUnitOfWork unitOfWork) : IRequestHandler<CompleteOrderCommand, Result<OrderResponse>>
{
    public async Task<Result<OrderResponse>> Handle(CompleteOrderCommand command, CancellationToken ct)
    {
        ...
        order.Complete(); await unitOfWork.SaveChangesAsync(ct); return Result.Success((await orders.GetResponseByIdAsync(order.Id, ct))!);
    }
}
