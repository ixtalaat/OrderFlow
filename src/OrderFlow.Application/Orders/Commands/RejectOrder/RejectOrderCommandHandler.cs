...
public sealed class RejectOrderCommandHandler(IOrderRepository orders, IInventoryRepository inventories, IUnitOfWork unitOfWork) : IRequestHandler<RejectOrderCommand, Result<OrderResponse>>
{
    public async Task<Result<OrderResponse>> Handle(RejectOrderCommand command, CancellationToken ct)
    {
        ...
        order.Reject(); await unitOfWork.SaveChangesAsync(ct);
