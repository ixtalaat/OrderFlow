...
public sealed class UpdatePricingRuleCommandHandler(IPricingRuleRepository rules, IUnitOfWork unitOfWork) : IRequestHandler<UpdatePricingRuleCommand, Result<PricingRuleResponse>>
{
    ...
    {
        var rule = await rules.GetByIdAsync(command.Id, ct); if (rule is null) return Result.Failure<PricingRuleResponse>(PricingErrors.NotFound);
        ...
        rule.Update(command.Tier, command.DiscountPercentage, command.ValidFromUtc, command.ValidToUtc); await unitOfWork.SaveChangesAsync(ct);
